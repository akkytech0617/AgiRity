...
droidにはread, write, fetchなどの内部ツールがあるので、必要に応じて活用してください。
まずは作業開始時に**droid-tool-master**というスキルが定義されているので、そのスキルで内部ツールの使い方を正しく把握し利用してください。
**このプロセスをまず絶対に行ってください**
...
| [docs/product/03_development_rules.md](docs/product/03_development_rules.md) | 開発ルール（ブランチ戦略、コミット規約） |
...
| [docs/implementation/e2e_testing_implementation_plan.md](docs/implementation/e2e_testing_implementation_plan.md) | E2E テスト導入計画 |
...
| [docs/planning/task_breakdown_template.md](docs/planning/task_breakdown_template.md) | タスク分解テンプレート |
