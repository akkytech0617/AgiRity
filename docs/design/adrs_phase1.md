...
> **Update (2025-12)**: このADRで決定したサイドバーレイアウトは、ブラウザライクなタブUIに変更されました。詳細は `docs/design/architecture.md` を参照してください。変更理由：
> - 小さいウィンドウサイズでのサイドバー表示問題
> - よりシンプルでスペース効率の良いUI
> - ブラウザライクな操作感の実現
> - Header: ロゴ＋ワークスペースタブ＋ハンバーガーメニュー構成に統一
> - quick-launchビューは削除、デフォルトでワークスペース詳細を表示
