...
export function WorkspaceDetail({ workspace, onLaunch }: { workspace: Workspace, onLaunch: (id: string) => void }) {
  const getItemIcon = (type: string) => {
    switch (type) {
      case 'app': return <Monitor className="w-5 h-5 text-blue-500" />;
      case 'browser': return <Globe className="w-5 h-5 text-green-500" />;
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
          {presets.map((preset, idx) => {
            ...
            return (
              <div 
                key={idx}
                className="bg-white rounded-xl border border-gray-200 p-5 shadow-sm hover:shadow-md hover:border-blue-300 transition-all group relative overflow-hidden"
              >
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-xs text-gray-500 mb-4 min-h-[2.5em]">{preset.description}</p>
                  
                  <div className="flex items-center gap-1 mb-5">
                     {presetItems.slice(0, 5).map((item, i) => (
                       <div key={i} className="p-1 bg-gray-50 rounded border border-gray-100" title={item.name}>
                         {/* Small icons for preview */}
                         {item.type === 'app' && <Monitor className="w-3 h-3 text-blue-500" />}
                         {item.type === 'browser' && <Globe className="w-3 h-3 text-green-500" />}
                         {item.type === 'folder' && <Folder className="w-3 h-3 text-yellow-500" />}
                       </div>
                     ))}
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            <h3 className="font-bold text-gray-900">Workspace Tools</h3>
            <span className="text-xs text-gray-500 bg-gray-100 px-2 py-1 rounded-full">{workspace.items.length} items</span>
          </div>
          <button className="flex items-center gap-1.5 px-3 py-1.5 bg-white border border-gray-200 hover:border-blue-300 hover:text-blue-600 text-gray-600 rounded-lg text-xs font-medium shadow-sm transition-all">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
          {workspace.items.map((item, idx) => (
            <div 
              key={idx} 
              className="bg-white p-3 rounded-lg border border-gray-200 shadow-sm hover:shadow-md hover:border-blue-200 transition-all group flex flex-col h-full"
            >
